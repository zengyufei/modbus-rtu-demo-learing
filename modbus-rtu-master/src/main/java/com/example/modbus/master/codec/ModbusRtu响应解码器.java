 ...
 * 主站侧 RTU 响应解码器。
 */
public class ModbusRtu响应解码器 extends ChannelInboundHandlerAdapter {
    ...
    /**
     * 输入：串口持续送入的字节流缓冲区。
     * 输出：从缓冲区中切出一帧或多帧完整 RTU 响应帧，并交给后续业务处理器。
     */
    private void 解码帧(ChannelHandlerContext 上下文) {
        while (累积缓冲.readableBytes() >= 4) {
            byte 从站地址 = 累积缓冲.getByte(累积缓冲.readerIndex());
            ...
            上下文.fireChannelRead(new ModbusRtu帧(从站地址, 功能码, 数据区));
        }
    ...
    private int 计算期望长度(ByteBuf 输入缓冲, byte 功能码) {
        int 功能码整数 = 功能码 & 0xFF;
        if (功能码整数 == 0x03) {
            if (输入缓冲.readableBytes() < 3) {
                return Integer.MAX_VALUE;
            }
            int 数据字节数 = 输入缓冲.getByte(输入缓冲.readerIndex() + 2) & 0xFF;
            return 3 + 数据字节数 + 2;
        }
        if (功能码整数 == 0x06 || 功能码整数 == 0x10) {
            return 8;
        }
        if ((功能码整数 & 0x80) != 0) {
            return 5;
    ...
    private void 丢弃残帧(String 原因) {
        if (累积缓冲 != null && 累积缓冲.isReadable()) {
            ...
            累积缓冲.readBytes(残留字节);
            累积缓冲.clear();
            System.err.printf("主站解码器丢弃残缺 RTU 帧，原因=%s，字节=%s%n", 原因, 转十六进制(残留字节));
        }
    }
